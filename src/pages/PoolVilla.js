        ...
        const reqRequestRoom = async () => {
          ...
          try {
            ...
            if(response.data.code === 1){
                ...
                imgList.push({id:"1" , size:"1152-768" , src: urlServer + response.data.list.imgthumb , thumb:urlServer+ response.data.list.imgthumb })
      
                  if(response.data.list.imglist.length > 0 ){
      
                    response.data.list.imglist.forEach((itm , key) => {
      
                      imgList.push({id:(key+2).toString() , size:"1152-768" , src: urlServer+ itm , thumb:urlServer+ itm})
