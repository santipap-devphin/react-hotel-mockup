...
const RoomEdit = () => {
        ...
        const reqRoomPerID = async () => {
                ...
                try {
                    ...
                    if(response.length === 4){
                        ...
                        setFileTempFront({filebase64:"" , fileimg:'http://localhost:7070/'+response[3].data.list.imgthumb , filepos:"front"})

                        response[3].data.list.imglist.forEach((name  , key) => {
                            
                            imgAr.push({filebase64:"" ,fileimg:'http://localhost:7070/'+name, filepos:"small" });
                        })
                        //console.log(imgAr)
                        setImgOldData(imgAr);
                        setStatusReq(true);
    ...
    const submitRoom = async (e) => {
        e.preventDefault();
        if(rCate && sizeRoom && sizeAdults && detailRoom && chkSwitch){
            ...
            imgOldData.forEach((data , key) => {
                    ...
                    if(data.filebase64 === ""){
                        var sp_url = data.fileimg.split("http://localhost:7070/");
