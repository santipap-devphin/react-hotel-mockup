...
const EventsEdit = () => {
    ...
    const [tagsAll , setTagAll] = useState(['เชียงใหม่' , 'แอดเวนเจอร์' , 'ร้านอาหารแนะนำ' ,'อันซีนไทยแลนด์' , 'อุทยานแห่งชาติ']);
     ...
     if(cateEvent && titleEvent && fileTemp.length > 0 && detailEvent.length > 0 && chipData.length > 0){
         ...
         try {
             ...
             if(response.data.code === 1){
                 ...
                 setSwalProps({...swalProps , show:true , title:"แก้ไขข้อมูลเรียบร้อยแล้ว" , showConfirmButton:false})
